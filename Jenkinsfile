 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  switch (env.BRANCH_NAME) {
    ...
    case "canary":
        ...
        sh("kubectl --kubeconfig ~admin12/.kube/config --namespace=dev apply -f ./services/")
        sh("kubectl --kubeconfig ~admin12/.kube/config --namespace=dev apply -f ./canary/")
    ...
    case "master":
        ...
        sh("kubectl --kubeconfig ~admin12/.kube/config --namespace=dev apply -f ./services/")
        sh("kubectl --kubeconfig ~admin12/.kube/config --namespace=dev apply -f ./production/")
